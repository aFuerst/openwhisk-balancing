 ...
 */
class EnhancedShardingContainerPoolBalancer(
  config: WhiskConfig,
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      val hash = EnhancedShardingContainerPoolBalancer.generateHash(msg.user.namespace.name, action.fullyQualifiedName(false))
      val homeInvoker = hash % invokersToUse.size
      val stepSize = stepSizes(hash % stepSizes.size)
      val invoker: Option[(InvokerInstanceId, Boolean)] = EnhancedShardingContainerPoolBalancer.schedule(
...
object EnhancedShardingContainerPoolBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(implicit actorSystem: ActorSystem,
    ...
    new EnhancedShardingContainerPoolBalancer(
      whiskConfig,
      instance,
  ...
  def underLoad(invoker: InvokerHealth, schedulingState: RedisAwareLoadBalancerState, lbConfig: ShardingContainerPoolBalancerConfig)(implicit logging: Logging, transId: TransactionId) : Boolean = {
  ...
  def schedule(
    ...
    if (numInvokers > 0) {
      ...
      if (invoker.status.isUsable && !(underLoad(invoker, schedulingState, lbConfig) && longWarmTime(strName, schedulingState)) && dispatched(invoker.id.toInt).tryAcquireConcurrent(fqn, maxConcurrent, slots)) {
        if (stepsDone > 0) {
